...
let eval_add arg1 arg2 =
  ...
  | Atom x, Atom y -> Atom (string_of_int (int_of_string y +  int_of_string x))
  |  _ -> raise (Error "addition only take two Atoms")
  
(* interpret (+ arglist) *)
let rec eval_add_list arglist =
  match arglist with
  | Atom "nil" -> Atom "0"
  | Atom x -> Atom (string_of_int (int_of_string x))
  | Cons (car,cdr) ->
     begin
       match car with
       | Atom x ->
          begin
            match eval_add_list cdr with
            | Atom j -> Atom (string_of_int ((int_of_string x) + (int_of_string j)))
            | _ -> raise (Error "wrong addition format")
          end 
       | _ -> raise (Error "addition only take Atom arguments")
     end 
...
let eval_mul arg1 arg2 =
  match arg1,arg2 with
  | Atom x, Atom y -> Atom (string_of_int (int_of_string y *  int_of_string x))
  |  _ -> raise (Error "multiplication only take two Atoms")

        
(* interpret (- arg1 arg2) *)
let eval_sub arg1 arg2 =
  match arg1,arg2 with
  | Atom x, Atom y -> Atom (string_of_int (int_of_string x -  int_of_string y))
  |  _ -> raise (Error "substraction only take two Atoms")

        
(* interpret (> arg1 arg2) *)
let eval_gr arg1 arg2 =
  match arg1,arg2 with
  | Atom x, Atom y -> Atom ( if int_of_string x >  int_of_string y then "t" else "nil")
  |  _ -> raise (Error "inequality only take two Atoms")
...
let all_builtins =
  [ (* Special forms *)
    ...
    "gr", BuiltinFn2 eval_gr;
    "le", BuiltinFn2 eval_le;
    "addl", BuiltinFn1 eval_add_list;
  ]
